...
GRAVITY_ACCEL = 9.81
B = 2.897771955 # Wien Displacement Constant, mm K
...
def calculate_energy_cost(node1: EnvironmentNode, node2: EnvironmentNode, robot_data_obj: RobotData) -> float:
    ...
    combined_motor_torque = calculate_combined_motor_torque(robot_mass, wheel_angular_velocity, wheel_radius, incline_angle_deg, force_of_gravity, kinetic_friction)
...
def calculate_combined_motor_torque(robot_mass: float, angular_velocity: float, wheel_radius: float, incline_angle_deg: float, gravitation_force: float, friction_force: float) -> float:
    incline_angle_rad = math.radians(incline_angle_deg)
    return ((robot_mass/2) * angular_velocity * pow(wheel_radius, 2)) + gravitation_force + friction_force

# Energy-specific calculations

def calculate_voltage(x_coord: int, y_coord: int, incline_angle_deg: float) -> float:
    incline_angle_rad = math.radians(incline_angle_deg)

    voltage = x_coord * math.sin(incline_angle_rad) - y_coord * math.cos(incline_angle_rad) + incline_angle_rad
    pass
